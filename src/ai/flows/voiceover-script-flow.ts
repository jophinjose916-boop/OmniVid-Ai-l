'use server';
/**
 * @fileOverview A flow to generate an engaging voiceover script from a video prompt, supporting Malayalam, English, and German.
 */
...
const ScriptGenerationInputSchema = z.object({
  ...
  targetLanguage: z.enum(['malayalam', 'english', 'german']).describe('Target language for the script.'),
});
...
2. Malayalam: Use beautiful, cinematic Malayalam (മലയാളം) that sounds natural and poetic.
3. English: Use atmospheric and engaging English.
4. German: Use professional, sophisticated, and evocative German (Deutsch).
5. Ensure the script matches the visual intent of the prompt.
6. If the input was in a different language, translate the sentiment poetically.`,
});
